...
############### It is identical to AbstractBaseNet
class AbstractBaseNetNEW(ABC):
    def __init__(self,
                 recon_coeff=RECON_COEFF,
                 class_coeff=CLASS_COEFF,
                 fair_coeff=FAIR_COEFF,
    ...
    @abstractmethod
    def _get_sensitive_logits(self, latents, scope_name='model/aud',
                              *args):  # map latents to sensitive logits (i.e., the adversary/auditor)
        pass
...
############### It is identical to DemParGan with XC added as a placeholder
class DemParGanNEW(AbstractBaseNetNEW):
    def _define_vars(self):
        assert (
                isinstance(self.hidden_layer_specs, dict) and
                all([net_name in self.hidden_layer_specs for net_name in ['enc', 'cla', 'aud', 'rec']])
        ...
        self.Y = tf.placeholder("float", [None, self.ydim], name='Y')
        self.A = tf.placeholder("float", [None, self.adim], name='A')
        self.XC = tf.placeholder("float", [None, self.adim], name='XC')
        self.epoch = tf.placeholder("float", [1], name='epoch')
        return
...
############### It is identical to EqOddsUnweightedGan
class EqOddsUnweightedGanNEW(DemParGanNEW):
    """Like DemParGan, but auditor gets to use the label Y as well"""

    def _get_aud_inputs(self):
        return tf.concat([self.Z, self.XC], axis=1)

    def _get_sensitive_logits(self, inputs, scope_name='model/aud', reuse=False):
        with tf.variable_scope(scope_name, reuse=reuse):
            mlp = MLP(name='latents_to_sensitive_logits',
                      shapes=[self.zdim + 1] + self.hidden_layer_specs['aud'] + [self.adim],
                      activ=ACTIV)
            return mlp.forward(inputs)
...
############### It is identical to WassGan
class WassGanNEW(AbstractBaseNetNEW):
    """Uses the Wasserstein loss function"""
...
############### It is identical to WeightedGan with weights AXc are indluded
class WeightedGanNEW(AbstractBaseNetNEW):
    """
    Weighted means we are going to scale each component of the loss according to the
    relative size of the group (A=0 or A=1) it belongs to. this is how we implement a group-normalized loss fn
    """
    ...
    def __init__(self,
                 ...
                 Y_weights=Y_WTS,
                 AY_weights=AY_WTS,
                 AXC_weights=AXC_WTS, ########################## weights for the new experiment
                 INDEX = 0,
                 **kwargs):
        self.A_weights = A_weights
        self.Y_weights = Y_weights
        self.AY_weights = AY_weights
        self.AXC_weights = AXC_weights ########################## weights for the new experiment
        self.INDEX = INDEX ########################## index of Xc
        super().__init__(recon_coeff, class_coeff, fair_coeff, xdim, ydim, zdim, adim, hidden_layer_specs, seed=seed,
                         hinge=hinge, **kwargs)
        self.unweighted_aud_loss = self._get_aud_loss(self.A_hat, self.A)
        self.aud_loss = self._get_weighted_aud_loss(self.unweighted_aud_loss, self.A_weights, self.Y_weights,
                                                    self.AY_weights, self.AXC_weights) ########################## weights for the new experiment are included
        self.loss = self._get_loss()

    @abstractmethod
    def _get_weighted_class_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):  # weight class loss for final loss function ########################## weights for the new experiment are included
        pass

    @abstractmethod
    def _get_weighted_recon_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):  # weight recon loss for final loss function ########################## weights for the new experiment are included
        pass

    @abstractmethod
    def _get_weighted_aud_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):  # weight auditor loss for final loss function  ########################## weights for the new experiment are included
        pass
...
############### It is identical to WeightedDemParGan with weights AXc are indluded
class WeightedDemParGanNEW(WeightedGanNEW, DemParGanNEW):
    def _weight_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):
        A0_wt = A_wts[0]
        A1_wt = A_wts[1]
        wts = A0_wt * (1. - self.A) + A1_wt * self.A
        wtd_L = tf.multiply(L, tf.squeeze(wts))
    ...
    def _get_weighted_class_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):
        return self._weight_loss(L, A_wts, Y_wts, AY_wts, AXC_wts)

    def _get_weighted_recon_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):
        return self._weight_loss(L, A_wts, Y_wts, AY_wts, AXC_wts)

    def _get_weighted_aud_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):
        return self._weight_loss(L, A_wts, Y_wts, AY_wts, AXC_wts)
...
############### It is identical to WeightedEqoddsGan with weights AXc are indluded
class WeightedEqoddsGanNEW(WeightedDemParGanNEW, EqOddsUnweightedGanNEW):
    def _weight_loss(self, L, A_wts, Y_wts, AY_wts, AXC_wts):
        A0_Y0_wt = AY_wts[0][0]
        A0_Y1_wt = AY_wts[0][1]
        A1_Y0_wt = AY_wts[1][0]
        ...
        A0_XC0_wt = AXC_wts[0][0]
        A0_XC1_wt = AXC_wts[0][1]
        A1_XC0_wt = AXC_wts[1][0]
        A1_XC1_wt = AXC_wts[1][1]

        wts = A0_XC0_wt * tf.multiply(1 - self.A, 1 - self.XC) \
              + A0_XC1_wt * tf.multiply(1 - self.A, self.XC) \
              + A1_XC0_wt * tf.multiply(self.A, 1 - self.XC) \
              + A1_XC1_wt * tf.multiply(self.A, self.XC)
        wtd_L = tf.multiply(L, tf.squeeze(wts)) ########################## new calculations of the adversary objective using the new weights

        return wtd_L

    def _get_sensitive_logits(self, latents, scope_name='model/aud', reuse=False):
        return EqOddsUnweightedGanNEW._get_sensitive_logits(self, latents, scope_name, reuse)
        # return EqOddsUnweightedGan._get_sensitive_logits(self, latents, scope_name='model/aud', reuse=False)
...
############### It is identical to WeightedEqoddsWassGan but it is based on WassGanNEW and WeightedEqoddsGanNEW
class WeightedEqoddsWassGanNEW(WassGanNEW, WeightedEqoddsGanNEW):
    def _get_class_loss(self, Y_hat, Y):
        return WassGan._get_class_loss(self, Y_hat, Y)
...
############### It is identical to RegularizedFairClassifier with XC added as a placeholder (the model use DemParGanNEW)
class RegularizedFairClassifierNEW(DemParGanNEW):
    """
    disparate impact-regularized classifier
    we typically invoke this during transfer_learn.py with fair_coeff=0, i.e., naiver classifier
    """
