...
class Tester(object):
    def __init__(self, model, data, sess, reslogger):
        ...
        file_data = [] ####### read the new.txt file to get the experiment type and the index
    ...
    ################# This function is identical to the evaluate function, but it also includes the Xc tensor
    def evaluateNEW(self, mb_size, phase='test', save=True):
        ...
        Ys = np.empty((0, 1))
        As = np.empty((0, 1))
        if self.NEW: ######### if new experiment (with xc), initialize the Xc tensor
            Xc = np.empty((0, 1))

        for x, y, a, xc in test_iter: ######### if new experiment (with xc), load the Xc tensor
            num_batches += 1
            if len(x) < mb_size:  # hack for WGAN-GP training; don't process weird-sized batches
                continue

            # make feed dict ########### add the Xc tensor to the feed dict
            feed_dict = {self.model.X: x, self.model.Y: y, self.model.A: a, self.model.XC: xc, self.model.epoch: np.array([BIG_EPOCH])}
            # make feed dict
            ...
            class_loss, recon_loss, class_err, Y_hat, Z, Y, XC = self.sess.run(
                [self.model.class_loss,
                 ...
                 self.model.Z,
                 self.model.Y,
                 self.model.XC], ########### get the Xc tensor
                feed_dict=feed_dict
                )
            # run forward auditor
            if self.NEW:
            ...
            if self.NEW: ######### if new experiment (with xc), concatenate the Xc tensor
        ...
        tensorD['A_hat'] = A_hat
        tensorD['A'] = A
        if self.NEW: ######### if new experiment (with xc), add the Xc tensor to the tensor dictionary
            tensorD['XC'] = Xc
        ...
        dp = DP(Y_hat, A)
        if self.NEW: ######### if new experiment (with xc), calculate the DXC metric
            DXC = DI(Xc , Y_hat , A)

        if self.NEW: ######### if new experiment (with xc), print the DXC metric
            metrics_str = 'Error Rate: {:.3f},  DP: {:.3f}, DXC: {:.3f}, DI: {:.3f}, di_FP: {:.3f}, di_FN: {:.3f}'.format(err, dp, DXC,di, difp,difn) \
                          + '\nError Rate (A): {:.3f}'.format(err_a)
        else:
            metrics_str = 'Error Rate: {:.3f},  DP: {:.3f}, DI: {:.3f}, di_FP: {:.3f}, di_FN: {:.3f}'.format(err, dp,di, difp, difn) \
                          + '\nError Rate (A): {:.3f}'.format(err_a)
        print(metrics_str)
        ...
        if self.NEW: ######### if new experiment (with xc), calculate the error mask for Xc
            metD['DXC'] = DXC
    ...
    def evaluate(self, mb_size, phase='test', save=True):
        ...
        print('\nPredicting Y')
        for mask, mask_nm in [(np.ones_like(A), 'A=?'), (A, 'A=1'), (1 - A, 'A=0'), (1 - errMaskA, 'ACor'),
                              (errMaskA, 'AWro')]:
            pr_base = subgroup(PR, mask, Y)
            nr_base = subgroup(NR, mask, Y)
            pr = subgroup(PR, mask, Y_hat)
            ...
            s = '{}: Base PR: {:.3f}, Base NR: {:.3f},  PR: {:.3f}, NR: {:.3f}, Err: {:.3f}, '.format(mask_nm, pr_base,
                                                                                                      nr_base, pr, nr,
                                                                                                      err) \
                + 'TPR: {:.3f}, FNR: {:.3f}, TNR: {:.3f}, FPR: {:.3f}'.format(tpr, fnr, tnr, fpr)
            print(s)
        ...
        for mask, mask_nm in [(np.ones_like(A), 'Y=?'), (Y, 'Y=1'), (1 - Y, 'Y=0'), (1 - errMaskY, 'YCor'),
                              (errMaskY, 'YWro')]:
            pr_base = subgroup(PR, mask, A)
            nr_base = subgroup(NR, mask, A)
            pr = subgroup(PR, mask, A_hat)
            ...
            s = '{}: Base-A1-rate: {:.3f}, Base-A0-rate: {:.3f}, Pred A1-rate: {:.3f}, Pred-A0-rate: {:.3f}, Err: {:.3f}, '.format(
                mask_nm, pr_base, nr_base, pr, nr, err) \
                + 'A1-Correct: {:.3f}, A1-Wrong: {:.3f}, A0-Correct: {:.3f}, A0-Wrong: {:.3f}'.format(tpr, fnr, tnr,
                                                                                                      fpr)
            print(s)
