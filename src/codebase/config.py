...
####################### ConfigParseNEW is a process the part related to the type of the experiment (New or not)
####################### ConfigParseNEW saves the results (type of the experiment and
####################### the index of Xc) to file new.txt that is will be accessed by other scripts
class ConfigParseNEW(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        options_dict = {}
        for overrides in values.split(','):
            k, v = overrides.split('=')
            ...
            for key in k_parts[:-1]:
                dic = dic.setdefault(key, {})
            dic[k_parts[-1]] = v
        if(options_dict['new']==True):
            with open("new.txt", "w") as f:
                f.write(str(1) + "\n")
                f.write(str(options_dict['index']) + "\n")
        else:
            with open("new.txt", "w") as f:
                f.write(str(0) + "\n")
                f.write(str(options_dict['index']) + "\n")
...
def get_config_overrides():
    ...
    parser.add_argument('-n', action=ConfigParseNEW,
                        help='NEW Model')
