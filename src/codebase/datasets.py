...
class Dataset(object):
    ...
    def get_AXC_proportions(self, index):
        ttl = float(self.y_train.shape[0])
        A0XC0 = TN(np.where(self.x_train[:,index]>0,1,0), self.attr_train) / ttl
        A0XC1 = FN(np.where(self.x_train[:,index]>0,1,0), self.attr_train) / ttl
        A1XC0 = FP(np.where(self.x_train[:,index]>0,1,0), self.attr_train) / ttl
        A1XC1 = TP(np.where(self.x_train[:,index]>0,1,0), self.attr_train) / ttl
        return [[A0XC0, A0XC1], [A1XC0, A1XC1]]
...
####################### It is a copy of Dataset with the addition of Xc when splitting the dataset #######################
####################### It is used in the new experiments #######################
####################### Xc is changed to a binary attribute #######################
class DatasetNEW(object):
    def __init__(self, name, attr0_name, attr1_name, npzfile, seed=0, use_attr=False, load_on_init=True, y2i=None,
                 pred_attr=False, batch_size=None, **kwargs):
        self.name = name
        self.attr0_name = attr0_name
        self.attr1_name = attr1_name
        self.npzfile = npzfile
    ...
    def load(self):
        if not self.loaded:
            ...
            self.x_train = dat['x_train']
            self.x_test = dat['x_test']
            self.x_train[:,self.INDEX] = np.where(self.x_train[:,self.INDEX]>0,1,0) ############# to binary
            self.x_test[:, self.INDEX] = np.where(self.x_test[:, self.INDEX] > 0, 1, 0) ############# to binary

            self.xc_train = self.x_train[:,self.INDEX].reshape(-1, 1) ############# add to train dataset
            self.xc_test = self.x_test[:, self.INDEX].reshape(-1, 1) ############# add to test dataset
    ...
    def make_validation_set(self, force=False):
        if not hasattr(self, 'x_valid') or force:
            ...
            self.y_valid = self.y_train[self.valid_inds]
            self.attr_valid = self.attr_train[self.valid_inds]
            self.xc_valid = self.xc_train[self.valid_inds]

            self.x_train = self.x_train[self.train_inds]
            self.y_train = self.y_train[self.train_inds]
            self.attr_train = self.attr_train[self.train_inds]
            self.xc_train = self.xc_train[self.train_inds]
            if hasattr(self, 'y2_valid'):
                self.y2_valid = self.y2_train[self.valid_inds]
                self.y2_train = self.y2_train[self.train_inds]
        # hack for WGAN-GP training: trim to bacth size if a batch size is specified
        if self.batch_size is not None:
            ...
            self.xc_train = mb_round(self.xc_train, self.batch_size)
            self.xc_test = mb_round(self.xc_test, self.batch_size)
            self.xc_valid = mb_round(self.xc_valid, self.batch_size)
    ...
    def get_AXC_proportions(self, index):
        ttl = float(self.y_train.shape[0])
        A0XC0 = TN(self.xc_train, self.attr_train) / ttl
        A0XC1 = FN(self.xc_train, self.attr_train) / ttl
        A1XC0 = FP(self.xc_train, self.attr_train) / ttl
        A1XC1 = TP(self.xc_train, self.attr_train) / ttl
        return [[A0XC0, A0XC1], [A1XC0, A1XC1]]
    ...
    def get_batch_iterator(self, phase, mb_size):
        ...
        sz = x.shape[0]
        batch_inds = make_batch_inds(sz, mb_size, self.seed, phase)
        iterator = DatasetIterator([x, y, a, xc], batch_inds)
        return iterator
...
####################### TransferDatasetNEW is a new dataset handler that takes Xc into account #######################
####################### It is a copy of TransferDataset with the addition of Xc when splitting the dataset #######################
####################### It is used in the new experiments #######################
class TransferDatasetNEW(DatasetNEW):
    def __init__(self, reprs, A, XC, label_index, Y_loaded=None, phase='Test', **data_kwargs):
        ...
        x_train, x_test, y_train, y_test, a_train, a_test, xc_train, xc_test = self.make_train_test_split(reprs, A, Y, XC)
        ...
        self.attr_train = a_train
        self.attr_test = a_test
        self.xc_train = xc_train ############# xc is considered as in input to be inserted to the dataset (it will be used to evaluate the fairness of the maive classifier
        self.xc_test = xc_test ############# xc is considered as in input to be inserted to the dataset (it will be used to evaluate the fairness of the maive classifier
        if ((self.xc_train==self.y_train).all()):
    ...
    def make_train_test_split(self, X, A, Y, XC):
        print(X.shape, A.shape, Y.shape)
        tr_inds, te_inds = self.make_valid_inds(X, pct=0.3)
        X_tr = X[tr_inds,:]
        X_te = X[te_inds,:]
        ...
        XC_tr = XC[tr_inds,:]
        XC_te = XC[te_inds,:]
        return X_tr, X_te, Y_tr, Y_te, A_tr, A_te, XC_tr, XC_te
