...
from codebase.datasets import TransferDataset, Dataset, TransferDatasetNEW
from codebase.models import RegularizedFairClassifier, RegularizedFairClassifierNEW
...
def main(args):
    ...
    if args['transfer']['repr_name'] == 'default':
        ...
        ###################### If new experiment (with xc), load the XC data #######################
        if NEW:
            args['model'].update({'class': 'RegularizedFairClassifierNEW'})
            XC_fname = os.path.join(npzdname, 'XC.npz')
            XC_dat = np.load(XC_fname)
            XC = XC_dat['X']
            # print(CX)
            file_data = []
    ...
    for label_index in y_indices:  # this will either be a list of ints or just 'a'
        if NEW: ############# if new experiment (with xc), use the new dataset  TransferDatasetNEW and model  RegularizedFairClassifierNEW #############

            data = TransferDatasetNEW(reprs, attrs, XC=XC, label_index = label_index, npzfile=npzfile, Y_loaded=y, phase=repr_phase, **args['data'], batch_size=args['train']['batch_size'])
            model = RegularizedFairClassifierNEW(**args['model'])
        else:
            data = TransferDataset(reprs, attrs, label_index, npzfile=npzfile, Y_loaded=y, phase=repr_phase,
                                   **args['data'], batch_size=args['train']['batch_size'])
        ...
        with tf.Session() as sess:
            ...
            if NEW: ############### if new train with the new function trainNEW ############
                trainer.trainNEW(**args['train'])  # , train_metric=train_metric)
            else:
                trainer.train(**args['train']) #, train_metric=train_metric)

            # test model
            tester = Tester(model, data, sess, reslogger)
            if NEW: ############### if new experiment (with xc), evaluate the model with the new function evaluateNEW ############
